...
当前生产 Worker 已接入 Google News RSS 主题查询，覆盖通信、A 股半导体、美股半导体、Oracle 和工信部站点发现。每个条目保存发布者、发布时间、短摘要和链接，并明确标记为 `discovery`。Google News RSS 是无鉴权发现入口，不是稳定契约；失败时页面显示降级或不可用，不用旧示例新闻替代。
