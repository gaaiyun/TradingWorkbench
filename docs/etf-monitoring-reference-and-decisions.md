...
A 股 5m 的合法时间域固定为上海 `09:30–11:30 / 13:00–15:00`。Yahoo 午休记录以及 `11:30 / 15:00` 的零成交平盘哨兵在采集层拒绝；读取层按每个交易日选择单一来源。存在至少 24 条合法记录的候选时按腾讯 → 东方财富 → Yahoo 排序；候选均不完整时按最近抓取时间、来源优先级和有效行数选择。migration `0019_remove_invalid_cn_intraday_bars.sql` 只清理 Yahoo、A 股、5m 的午休行和零成交平盘端点，不删除真实成交收盘柱、其它来源、其它周期或美股记录。
...
的实际数值仍必须与对应 `I#` 完全一致。
历史审计也遵循当前公开边界：旧 manifest 若声称 rated/verified 却记录过滤过不安全
段落，会被当前索引降为 `invalidated`；已人工确认的旧语义绕过按报告路径失效。报告
失败后不得向后搜索这些旧评级来填充“主题结论”。
...
只读检查确认，该站点的 `/api/market-data/stocks` 和 `/api/market-data/factors` 把
`stock_signal_features` 表统计为 1092 只股票、957,684 行和 103 个因子。它是对方内部
筛选股票池和特征表，不是交易所全市场接口，也没有进入本项目生产数据。

可借鉴的是接口与纪律：因子公式字典、signal date 与 executed date 分离、费用与滑点、
ETF 日内成交额相对历史同时间桶、市场环境完整度和 source health。不可照搬的是其股票池、
私有历史、采样式 1m/5m 数据和无法验证的组合回测口径。TradingWorkbench 当前只实现两种
有界日线校验规则；全市场组合和 103 因子研究在历史成员、公司行动和点时财务数据具备前
不会上线。
