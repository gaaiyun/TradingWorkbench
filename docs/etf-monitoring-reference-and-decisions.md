...
参考“桃子冰粉的数据看板”的是信息设计和验证方法，不是“国家队买卖”结论：把绝对值配成历史分位、区分动作力度与绝对水平、用多个标的交叉检查、主动隐藏不能进入决策的噪音。当前三个核心标的是行业 ETF，公开数据不足以把份额变化归因到汇金、证金、险资或具体机构，因此产品的叙事主语限定为“杠杆资金”和“申赎资金（份额变化代理）”。这两类都是可观察行为，不是投资者身份识别。
...
没有找到可免费、稳定回填 `159995.SZ` 日频份额的同等级官方源。天天基金 `gmbd` 只有季度和特殊时点，东财 ETF 列表与集思录只有快照；这些结果用于交叉核验，不伪装成日频历史。上交所 commonQuery 对 GitHub/Cloudflare 出口存在间歇性 403，因此日常任务保留 source-level 降级：上交所失败时仍写入两融和当前份额快照。

分位规则固定为 2024-01-01 起、当前值不进入基准、并列值使用 mid-rank、有效历史少于 60 个交易日不输出 P 值。份额卡与对照图使用日度份额变化分位，绝对份额只作当前规模摘要；相邻份额变化超过 35% 的日期视为潜在拆分或口径变化，保留缺口但不参与分位。融资净买入和份额变化的正负决定流入/流出方向，分位只描述相对历史力度，不能反过来覆盖实际正负。融券和股指期货没有进入页面：前者在当前研究场景占比过小，后者与三个行业 ETF 的直接解释力不足；以后只有先得到稳定相关性证据才重新评估。

市场监控内的近 60 期双线图把融资净买入与日度份额变化分别转换成历史分位后比较，因此可以观察两类行为是否同向，但不能把两条线理解为同金额或同规模。图上的事件和官方新闻标记只回答“同期发生了什么”，不主张事件导致资金变化。页面一句话使用确定性规则，不调用模型，也不输出买卖建议。
