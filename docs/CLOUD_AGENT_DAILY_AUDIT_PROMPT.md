...
本提示词不绑定 Hermes 08:30 投资简报 cron。08:30 任务只产出只读盘前投资简报；本工程审计仅在人工触发时运行，用于发布验收、事故排查或专项复审。把下面“手工审计任务”整段交给具备网页访问、HTTP GET、GitHub 只读权限的云端 Agent。默认只读；只有用户明确授权“修复并上线”时，才执行后半部分的修复流程。不得读取、输出或猜测任何 token、access code、Cookie 或 Secret。
...
6. 最新柱与市场开闭状态一致，休市时旧收盘应标明，不得冒充实时；
   ...
   同一最新端点的 5m、15m、1h 顶层状态应一致；15m/1h 不能被聚合桶中较早记录的旧 stale 标记污染；
7. 自选列表“日涨跌”使用日线前收口径，不得跟随当前图表周期；
8. A 股红涨绿跌，美股/港股绿涨红跌；健康状态色不得与涨跌色混用；
9. 桌面和手机视口都检查七个一级入口、图表、tooltip、时间范围、无横向溢出和无 page error。

三只 A 股 ETF 还要逐一检查 5m：上海时间不得出现 `11:30 < time < 13:00` 的柱；Yahoo `11:30 / 15:00` 不得保留零成交平盘哨兵；同一标的同一交易日 API 不得混合多个 provider；真实有成交的 `15:00` 收盘柱必须保留。清理不得影响腾讯/东方财富、美股或非 5m 数据。
...
7. 原始 Agent 分卷可在 GitHub 为开发审计保留；claim validation 失败时，网页标签页、无 selector 与带身份的报告 API 都只能返回 fail-closed `complete_report.md`，不能公开原始角色分卷，也不能进入最新观点或问答上下文。还要对至少一份未验证报告直接请求 Pages raw 静态路径，必须不可读取；仅确认页面隐藏标签或 API 拒绝不够，因为原始文件可能仍随 artifact 发布；
...
15. 检查 `omittedUnsafeParagraphs` 及公开 Portfolio Decision：无引用定性叙事、窗口
    最低/最高/第 N 高、跌破面值、持续路径、价量因果和“相互独立指标”等表述必须
    有专门 Evidence，否则应被逐段剔除；raw Agent 分卷中的评级不能冒充 verified
    结论。反过来，“既非空头排列”“卖压是否释放无法确认”等否定或不确定表达不应
    被误报为正向事实。
16. 尝试四类绕过：Markdown 标题夹带结论、Rating 字段尾随理由、纯引用/纯条件/
    “下一步观察”空壳、用任意 N#/CA# 替无关因果背书；任一仍能成为 verified 即失败。
17. 否定作用域按局部从句检查：“不能据此推断资金流出”应允许；“不能忽视/无法否认/
    没有证据反驳主力资金流出”仍是正向归因，必须拒绝。
18. 若 Evidence 没有明确阈值、历史分位、统计检验或显式 `claimCapabilities`，拒绝
    “极端/异常/罕见”“无信号/随机/噪音”、指标一致提高置信度等定性推断；不得从
    单点波动率推导反弹、错失反转或清仓风险，也不得从公司行动标题推导“不损害利益”
...
报告首行标注“手工工程审计”，不得标成“08:30 投资简报”，也不得把简报是否送达作为部署、数据或报告门禁通过的证据。报告必须按以下顺序：
