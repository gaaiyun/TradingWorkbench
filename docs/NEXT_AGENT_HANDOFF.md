...
- 宇宙刷新以东财为主；东财在 GitHub runner 上整批失败时降级到新浪行情中心 `hs_a`，
  快照写入实际 source。两个免费源都失败时保留上一份快照，不写空覆盖。
- 首轮完整快照由 workflow `32048765793` 生成：A 股当前上市股票 `5543` 只，配置核心标的
  13 个，合计 `5556`；数据提交为 `9fae701`，实际源为 `sina-market-center`。该 run 的采集
  与提交成功，但 Pages dispatch 遭遇 GitHub API 瞬时 `503`；workflow 已增加 3 次有界重试，
  最终上线状态仍须以后续 Pages 成功 run 和生产 API 回读为准。
...
- 发布前必要回归为 Functions `444 passed / 1 skipped`、前端 `122/122`，脚本语法与资产版本
...
部署后第一次真实单标的 run（`31740098198`，20:17 UTC 触发）耗时 `1h11m11s`（远超
~10 分钟的正常基线），日志显示 `grok-4.5` 在 20:22–21:28 UTC 之间**先后 12 次**
命中瞬时错误，触发 `"Primary model 'grok-4.5' failed with a transient error;
retrying via fallback 'gpt-5.4-mini'."`——每一次都成功切到 `gpt-5.4-mini` 并继续，
最终 `[DONE] 1/1 tickers ok`，`Persist reports to main`、`Trigger Cloudflare Pages
deployment`、`Fail when analysis failed`（这次正确地没有触发）全部通过。生产
`/api/health` 复核为 `status:ok`，`reports.status:ok`，`deployment_manifest`
SHA 与本次发布一致。**这是当天 5 次真实调用里唯一一次产出完整报告的**——前 4 次
（524/524/503/524）在兜底上线前全部失败。

结论：
- Grok 故障转移机制在真实、持续（1 小时+、12 次独立失败）的生产故障下工作正常，
  不是一次侥幸；每次失败都被正确识别并接住，没有掩盖任何非瞬时性错误（日志里没有
  400/401 类误触发）。
- 中转站的这次故障本身持续时间显著长于早前几次（跨度超过 1 小时，此前三次真实
  失败共发生在约 16:00–17:14 UTC 的更短窗口内），说明这不是短暂抖动，接手时如果
  再看到大量 fallback 触发日志属于预期行为，不代表兜底本身有问题。
- 耗时 1h11m 远超正常水平，主要是 grok 每次失败前都要等到触发超时/5xx 才会切换
  （没有更短的探测式熔断），且中途反复来回失败——如果未来这类长时间中转站故障
  变得频繁，可以考虑给 grok 加一个"最近 N 次失败后本轮直接跳过 grok、直接用
  fallback"的短路，而不是每次都重新尝试；本轮未实现，只记录方向。
...
- GitHub `main` 与 Pages 均为 `1661679368412063779dc8527cafe6220c2a3f1f`，Pages `/api/health=status:ok`、`deployment_manifest.ok=true`。Monitor Worker 仍停留在 `23eb26b1a98ee2764b1dc95284990cf3091819d1`，原因是 08-03 后只有报告数据提交，没有触发 Worker 路径；本轮功能提交发布后必须再次回读三方完整 SHA。
- GOOGL 仍有 1 条合法 SEC 8-K evidence，原文位于 `sec.gov/Archives/edgar/data/...`；ORCL 最近窗口内仍为 0 条，但 `sec-edgar-submissions` 当前查询成功且没有 403。既有回归测试继续保证 SEC 403 会留下 `NEWS_HTTP_403`，且 discovery 成功不能掩盖官方源失败。
- `512480.SS` 已出现中国政府网《集成电路布图设计保护条例》原文 evidence，并保留 3 条上交所原始 PDF；`515880.SS` 保留 4 条上交所原始 PDF，本轮“通信”政策查询成功但没有新的合格政策原文，不能为凑数放宽门禁。`official-news` 最近仍有上交所 403 间歇失败，失败按标的留痕且不会删除既有 evidence。
- 三只 A 股 ETF 日线各 651 行，最新上海交易日为 2026-08-07，来源均为 Tencent、`adjustment=qfq`，重复日期、周末日期和非法 OHLCV 均为 0。聚合新闻允许同一文章映射到多个相关标的；同一标的内 API 按 cluster 合并并优先返回最新 `fetched_at` 的有效链接，前端再跨标的合并显示。08:25 没有产生完整分析 dispatch，最近已完成 monitor dispatch 的 slot/run 标识互不重复。
- 真实缺陷：HashKey 官网已迁移到 WordPress，旧 `https://group.hashkey.com/en/news/categories/announcement-1` 返回 404。现改用官方 RSS `https://group.hashkey.com/category/blog/news/feed/`，保留 HTTPS/官方域名校验、evidence 层级和旧嵌入式页面解析兼容；真实上游返回 200 且能解析最新公司公告。官网迁移造成的新旧 URL 同标题记录由 API 按同标的 cluster 折叠，优先保留本轮新采集的可用链接。新增 RED→GREEN 回归后，Functions `436 passed / 1 skipped`、frontend `121/121`、Python `694 passed / 2 skipped`（Windows 需 `PYTHONUTF8=1`）、Ruff 全绿。
