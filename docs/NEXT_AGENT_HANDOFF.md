...
- 三只 A 股 ETF 日线各 651 行，最新上海交易日为 2026-08-07，来源均为 Tencent、`adjustment=qfq`，重复日期、周末日期和非法 OHLCV 均为 0。聚合新闻返回中同一文章会按多个相关标的保存，但同一标的 URL 与 ID 无重复；前端按 cluster 合并显示。08:25 没有产生完整分析 dispatch，最近已完成 monitor dispatch 的 slot/run 标识互不重复。
- 真实缺陷：HashKey 官网已迁移到 WordPress，旧 `https://group.hashkey.com/en/news/categories/announcement-1` 返回 404。现改用官方 RSS `https://group.hashkey.com/category/blog/news/feed/`，保留 HTTPS/官方域名校验、evidence 层级和旧嵌入式页面解析兼容；真实上游返回 200 且能解析最新公司公告。新增 RED→GREEN 回归后，Functions `435 passed / 1 skipped`、frontend `121/121`、Python `694 passed / 2 skipped`（Windows 需 `PYTHONUTF8=1`）、Ruff 全绿。
...
| 2026-08-10 | 周一 08:25 后生产验收；修复 HashKey 官网迁移导致官方源持续 404，改用官方 WordPress RSS 并保留 evidence 与域名门禁 | 真实 RSS 200 且解析最新公告；Functions `435/1 skip`、frontend `121/121`、Python `694/2 skip`、Ruff 全绿；SEC、政策、上交所、新闻去重、dispatch 与 A 股 qfq 连续性按本节现场证据复核 |
