...
更新日期：2026-07-29（资金观察红框问题已修，待最终生产回读）
...
- 新增回归覆盖：驱动篮子、日线/盘中精度合并、陈旧盘中拒绝、规则观察 fallback、标题口径。当前前端 `118/118`、Functions `368 passed / 1 skipped / 0 failed`、完整浏览器脚本退出 0；最终上线仍需回读 GitHub、Pages、Worker SHA 并在生产红框位置复测。
...
| 2026-07-29 | 修复用户截图红框：标的专属隔夜驱动篮子、精确日涨跌、无 verified 报告时的规则化主题观察；Evidence 与问答门禁不变 | 前端 118/118、Functions 368/1 skip、浏览器脚本退出 0；生产运行号与 SHA 在本轮发布后补记 |
