...
- 首次发布后的真实复验 run `31648209934` 完整成功并生成 `2026-08-12-v2`，引用数由
  ...
  `UNSUPPORTED_DERIVED_NUMERIC_CLAIM`。因此第二阶段在生成端加入一次有界修订：先用同一
  确定性 Evidence validator 检查草稿，只把稳定错误码反馈给 Portfolio Manager 重写一次；
  要求数字逐字复制、拆短段。修订后仍失败时仍由原报告门禁拦截，不循环重试、不放宽规则。
