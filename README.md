...
- 定时 5 分钟采集每个标的只处理最近 96 根；D1 仍保留 90 天历史。5 分钟 freshness 按上海或纽约真实交易时段判断，午休、收盘和周末冻结在最近合法会话端点。腾讯会用区间结束时刻标注正在形成的 A 股 5 分钟柱，因此只容忍一个 5 分钟步长内的合法端点前置；更远的未来值和开盘期间真实延迟仍标为 `stale`。
...
- 公开报告按段落复用同一 Evidence validator：无引用的定性叙事、未预计算的窗口排名/极值、“跌破面值”、仅由价量推出的持续路径或因果关系，以及把同源技术指标称为“相互独立”，都会从公开结论中剔除并计入 `omittedUnsafeParagraphs`；原始 Agent 文本只留在审计分卷。被否定的表述（如“既非空头排列”“卖压是否释放无法确认”）不会误判为正向事实。
- 标题伪装、Rating 尾随理由、纯引用残片和只有条件/后续观察的空壳报告不能成为 verified；泛化的新闻或公司行动引用不证明无关因果，否定判断按局部从句生效。
- 没有专门阈值、历史分位或统计检验时，不得把数值描述为“极端/极高/显著”、把单日变化称为“无信号/噪音”，也不得用波动率推导反弹路径或清仓风险。
