...
期权页的“卖方策略观察”是确定性风险提示，不是自动下单。它按到期日把一日 VaR（缺失时才用 HV30 正态近似）缩放到工作日 √DTE：虚值距离低于 90% 尾部线直接标为“认怂”，达到 99% 尾部线才列为卖方观察候选；同时检查 IV-HV、价差和持仓量。IV 低于 HV 时明确提示“不宜裸卖”。如果上游没有逐合约 IV/Greeks，则仍显示 90%/99% 行权价距离和覆盖率，但不猜 Delta 档位、不生成裸卖指令。行情报价每 30 秒刷新，VaR/BSADF/HV 等慢指标按 5 分钟时钟刷新；页面始终分别显示行情时间与指标时间。
