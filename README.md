...
数据落入独立 `fund_flows` 表，经 `/api/flows` 读取；`/api/monitor-status?capacity=1` 会显示该表的有界行数。容量探针只在显式请求时扫描，默认超时 3000ms、硬上限 5000ms，不做超时重试；普通页面轮询不承担这项成本。当前资金面不进入 EvidencePacket、报告哈希或 verified 门禁。
...
Markdown 标题和 Rating 字段不能夹带分析结论；只剩评级、引用标记、免责声明、条件句
...
计算的收益率、比例、均线偏离和交易日数量一律 fail-closed；单时点指标也不能被描述
为“仍在扩张、尚未收敛或正在加速”。“空头排列”只在
`close < MA20 < MA60` 时成立，“多头排列”只在 `close > MA20 > MA60` 时成立。
...
2026-08-10 周一盘前验收确认 HashKey 官网路径已迁移：旧 IR 页面返回 404，官方源已切换到公司 WordPress RSS `https://group.hashkey.com/category/blog/news/feed/`。RSS 结果仍只接受 `group.hashkey.com` 的 HTTPS 原文并标记为 evidence；Google News/Yahoo 只作发现层降级。精确生产 SHA、来源状态和证据条目继续以两个 health 端点及 `/api/news` 实时回读为准。
...
2026-08-18 生产首刷取得 A 股当前上市股票 `5543` 只，实际降级源为新浪行情中心；历史
退市成员仍不可用，因此宇宙状态保持 `degraded`。CI `32049058200` 与 Pages
`32049125716` 成功，生产 `512480.SS` 回测读取 `528` 根 qfq 日线并完成 `58` 笔交易。
这些数字只证明数据链和时点约束可运行，不构成策略有效性或投资收益承诺。
...
- 定时 5 分钟采集每个标的只处理最近 96 根；D1 仍保留 90 天历史。5m 及其 15m/1h 聚合周期的 freshness 按上海或纽约真实交易时段和最新完成端点判断，不能让同一聚合桶内较早记录的旧状态污染当前柱；午休、收盘和周末冻结在最近合法会话端点。腾讯会用区间结束时刻标注正在形成的 A 股 5 分钟柱，因此只容忍一个 5 分钟步长内的合法端点前置；更远的未来值和开盘期间真实延迟仍标为 `stale`。
