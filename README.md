...
资金流 `ts` 采用“上海交易日 00:00”的 UTC 瞬时表示：例如 2026-07-27 交易日存为 `2026-07-26T16:00:00Z`。业务日期必须按 `Asia/Shanghai` 还原，禁止直接截 UTC 字符串前 10 位，否则会把正常的周一误读为周日。采集器拒绝上游周末记录，前端和验收脚本都按上海交易日检查。
