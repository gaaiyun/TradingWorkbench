...
- 发现层：Google News RSS；Cloudflare 出口不可用时，A 股使用东方财富，美股和港股使用 Yahoo Finance RSS。A 股政策发现要求行业词出现在标题，或标题明确来自国务院、发改委、工信部、证监会、财政部且摘要含行业词；仅在“风险提示”里顺带出现半导体/通信的其它行业文章会被拒绝。
